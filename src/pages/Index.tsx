...
const Index = () => {
  ...
  if (loading) {
    ...
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-purple-900 dark:to-blue-900 flex items-center justify-center px-4">
  ...
  return <ThemeProvider attribute="class" defaultTheme="light" enableSystem>
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-purple-900 dark:to-blue-900 safe-area-top safe-area-bottom">
        <div className="container mx-auto max-w-md lg:max-w-6xl min-h-screen">
          <div className="px-4 lg:px-8 py-6 space-y-4 lg:space-y-6 bg-transparent">
            <AppHeader userFirstName={profile?.first_name} avatarUrl={profile?.avatar_url} onSignOut={handleSignOut} />

            <AppNavigation activeView={activeView} onViewChange={setActiveView} />

            <AppContent activeView={activeView} purchases={purchases} onAddPurchase={addPurchase} onDeletePurchase={deletePurchase} onPurchaseSuccess={handlePurchaseSuccess} />
          </div>
        </div>
      </div>
    </ThemeProvider>;
